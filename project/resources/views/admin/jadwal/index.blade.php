...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <h1 class="text-xl font-bold text-gray-800 flex items-center gap-2">
            <i class="fas fa-calendar-alt text-blue-500"></i> Manajemen Jadwal
        </h1>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 font-medium text-gray-800">{{ $j->mata_kuliah }}</td>
                    <td class="px-4 py-3 text-gray-600">{{ $j->kelas }}</td>
                    <td class="px-4 py-3 text-gray-600">{{ $j->tanggal->format('d/m/Y') }}</td>
                    <td class="px-4 py-3 text-gray-600">
                        {{ \Carbon\Carbon::parse($j->jam_mulai)->format('H:i') }} –
                        {{ \Carbon\Carbon::parse($j->jam_selesai)->format('H:i') }}
                    </td>
                    <td class="px-4 py-3 text-gray-600">{{ $j->lokasi->name }}</td>
                    <td class="px-4 py-3">
                        @if($j->tanggal->isToday())
                            <span class="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-xs font-semibold">Hari Ini</span>
                        @elseif($j->tanggal->isPast())
                            <span class="bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full text-xs">Selesai</span>
                        @else
                            <span class="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full text-xs">Mendatang</span>
                        @endif
                    </td>
                    <td class="px-4 py-3">
                ...
                @empty
                <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400">Belum ada jadwal.</td></tr>
                @endforelse
            </tbody>
        </table>
...
<div id="modalTambah" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4">
        <div class="flex justify-between items-center mb-4">
            <h2 class="font-bold text-gray-800 text-lg">Tambah Jadwal</h2>
        ...
        <form method="POST" action="{{ route('admin.jadwal.store') }}" class="space-y-3">
            ...
            <div>
                ...
                <input type="text" name="mata_kuliah" required placeholder="cth: Web Programming Lanjutan"
            ...
            <div>
                ...
                <select name="lokasi_id" required class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-300 outline-none">
                    <option value="">-- Pilih Lokasi --</option>
                    @foreach($lokasis as $l)
                    <option value="{{ $l->id }}">{{ $l->name }} (radius {{ $l->radius_meter }}m)</option>
                    @endforeach
                </select>
            </div>
